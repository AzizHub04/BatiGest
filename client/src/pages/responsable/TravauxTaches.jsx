...
const TravauxTaches = () => {
  ...
  const { data: travaux = [], refetch: refetchTravaux } =
    useGetTravauxByChantierQuery(chantier?._id, { skip: !chantier });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editMode) {
        ...
        setSucces({ type: "warning", message: "Travail modifié avec succès" });
      } else {
        await creerTravail({ ...form, chantier: chantier._id }).unwrap();
        setSucces({ type: "success", message: "Travail créé avec succès" });
      }
      setModalOpen(false);
  ...
  const handleDelete = async (id) => {
    try {
      ...
      setSucces({ type: "delete", message: "Travail supprimé avec succès" });
      setTimeout(() => setSucces(null), 3000);
    } catch (err) {
      console.error(err);
    }
  ...
  return (
    <div>
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Travaux & Tâches</h2>
        <p className="text-sm text-gray-400 mt-1">
          Gérez l'avancement des travaux du chantier
        </p>
      </div>
